...
按 ROI 排序的推进站:**T0** 线上最小闭环 →(已落地)**T1** 数据接入层 →(已落地)**T2** auth →(已落地)**T3** 契约对账 →(已落地)**T5** 事件接收(接缝先行未公网暴露)→(已落地)**T6** 部署加固(与 T4 先后经 ROI 复评对调:T4 签名验真依赖客户端对端)→ **T4** 离线包(未落地,待客户端联调窗口)。每个推进站另起 session 用 design-gate-lite 锻造独立 PRD 后落地,PRD 落在 `product-requirements/<station>/`。
