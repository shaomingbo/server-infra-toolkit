...
>
> **实现期回写(2026-06-13)**:下列 Q1/Q2 均已在 T1 落地期定论(见各条 ✅ 标注);原问题文字保留作设计留痕。
...
- **Q1**:迁移工具最终选型?
  ...
  - **✅ 已关闭(T1 实现期定论)**:最终选 **goose**(jackc 系、与 pgx 同源,go 1.26 `tool` 指令锁在 `tools/` 独立 module),非推荐默认的 golang-migrate;契合本仓 sqlc 纯 SQL 工作流。
- **Q2**:Neon 当前套餐的连接数上限 + 用 pooler 端点还是直连端点?
  ...
  - **✅ 已关闭(T1 实现期定论)**:连接上限护栏已落地(`pool.go`:`defaultMaxConns=5`、硬上限 `maxConnsCeiling=20`,ceiling × 最多 2 实例 ≤ Neon 端点上限);端点 pooler/直连由 `NEON_DSN` 运维配置承载,代码用保守 ceiling 对两种端点都安全兜底。
