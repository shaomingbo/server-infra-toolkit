...
- **目标用户**:明博——server-infra-toolkit 的规划者 + 唯一实现者(熟 Node.js、Go 新手,单人开发 + AI 杠杆)。次要消费者:T1 落地的 implementer/reviewer;**首个数据层消费者 = T2 auth**(它要用连接池 + 事务写 refresh token)。
...
- **Q1**:迁移工具最终选型?
  - *为什么是明博的*:这是会被 T2-T8 所有有状态站继承的长期工具链 lock-in;且明博熟 Node.js(可能有 Prisma 声明式 / knex 命令式的迁移心智偏好),工具的「手感」是个人运维偏好。
